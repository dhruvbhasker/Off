...
def parse_copybook(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        for line in lines:
            ...
            match_05 = re.search(r'\s*05\s+([\w-]+)\s+PIC\s+(\w)\((\d+)\)', line)
            match_88 = re.search(r'\s*88\s+([\w-]+)\s+PIC\s+(\w)\((\d+)\)', line)
            ...
            elif match_88 and inside_05:
                current_05_length += int(match_88.group(3))
                new = current_05_length
