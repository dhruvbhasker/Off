...
//						appGeneratedCSV.convertCSVFromFile(strExtractName[index],"POST");						
	...
	private static ComparisonDataDTO getDTO(String strExtractName, boolean BlnFetchExtractUsingSQL) throws IOException {
		...
		File file = new File(currentDirectory+"\\AppGenExtractFiles\\PRE\\CSV_Files\\"+strExtractName+".csv"); 
...
//		}		
		...
		skipColumnsPos = "18,19";
		skipColumnsIndex = "1-"+columnCount;
//		skipColumnsIndex = (columnCount/3)+"-"+((columnCount/3)+(columnCount/3));
//		skipColumnsIndex = ((columnCount/3)+(columnCount/3))+"-"+columnCount;
		for(int i=0;i<columnCount;i++) {
			alActualSrcColumn.add(header[i]);
//			skipColumnsIndex = skipColumnsIndex + (i+1) +",";
		}
